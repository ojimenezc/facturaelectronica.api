...
public class InvoicingController extends BaseController {
    ...
    public GenerateDocumentResponse create(@RequestBody BillGenerationRequest request) throws Exception {
        ...
        try {
            ...
            loggerService.error("The document number " + facturaElectronica.getClave() + " was not successfully sent to hacienda. Error cause = " + ex.getErrorMessage());
            ...
            if (null != badRequest) {
                loggerService.error("The document number " + facturaElectronica.getClave() + " was not successfully sent to hacienda. Error cause = " + badRequest.getResponseHeaders().get("X-Error-Cause"));
